...
def delay_threshold_test(df, n_classes, platform, model_type):
    ...
    for delay_threshold in tqdm(np.arange(45, 270,1)):
        ...
        for i, data in df.groupby(['sample']):
            ...
            timings = data['overall time'].tolist()
            #cum_timings = np.cumsum(timings)
            exits = len([time for time in timings if time < delay_threshold])
    ...
    post_prediction.to_json('egde/' + platform + '_offload_' + model_type + '_analysis.json')
...
if __name__ == "__main__":
    ...
    with open('edge_test/nuc_2_jetson_' + args.model_type + '.json', 'r') as json_file:
        data = json.load(json_file)
